...
//专人干专事
/**封装 paramMapping，key:参数名称,value:参数在参数列表中的索引*/
public class GPHandlerAdapter {
    /*key:参数名称,value:参数在参数列表中的位置index*/
    ...
    /**为什么要把handler传进来，因为handler中包含了controller、method、url信息*/
    /**将请求url中参数值封装到 方法参数值数组中，然后反射调用method.invoke(controller,paramValues)*/
    public GPModelAndView handle(HttpServletRequest req, HttpServletResponse resp, GPHandlerMapping handlerMapping) throws  Exception {
        ...
        for (Map.Entry<String,String[]> param : reqParameterMap.entrySet()) {
            /*一个参数名称映射一个参数值的数组，此处将数组转换为字符串保存*/
            String value = Arrays.toString(param.getValue()).replaceAll("\\[|\\]","").replaceAll("\\s","");
            if(!this.paramMapping.containsKey(param.getKey())){continue;}
            /*获得命名参数 (@RequestParameter name) 在该方法的参数列表中的位置*/
            int index = this.paramMapping.get(param.getKey());
            //因为页面上传过来的值都是String类型的，而在方法中定义的类型是千变万化的
            //要针对我们传过来的参数进行类型转换
            paramValues[index] = this.caseStringValue(value,paramTypes[index]);
        }
        ...
        //4、从handler中取出controller、method，然后利用反射机制进行调用（调用@Controller类中的方法）
        Object result = handlerMapping.getMethod().invoke(handlerMapping.getController(),paramValues);
        if(result == null){ return  null; }
        /*handler.getMethod().getReturnType()：@Controller类中的方法的返回值类型*/
